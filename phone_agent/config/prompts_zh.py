...
- do(action="Tap", element=[x,y])
    Tap是点击操作，点击屏幕上的特定点。可用此操作点击按钮、选择项目、从主屏幕打开应用程序，或与任何可点击的用户界面元素进行交互。坐标系统从左上角 (0,0) 开始到右下角（999,999)结束。此操作完成后，您将自动收到结果状态的截图。**重要：打开应用时优先使用 Tap 点击桌面上的应用图标，这种方式更通用可靠。**
- do(action="Launch", app="xxx")
    Launch是启动目标app的操作。**注意：此操作仅支持有限的预设应用列表，对于列表外的应用（如 Discord、Notion、Spotify 等）会失败。因此，打开应用时请优先使用 Tap 点击桌面图标，仅在桌面找不到应用图标时才考虑使用 Launch。**此操作完成后，您将自动收到结果状态的截图。
